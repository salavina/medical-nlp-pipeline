...
class ModelTrainerHF(object):
    ...
    def _tokenize(self, row):
        return self.tokenizer(
            row["Findings"], truncation=True, padding="max_length", max_length=50
        )

    def load_dataset(self):
        dataset = dataset = load_dataset(
            path="csv",
            data_files=self.config.training_data
            + "Independent_Medical_Reviews_Custom.csv",
            split=Split.TRAIN,
        )
        return dataset
    ...
    def load_trainer(self):
        ...
        args.report_to = []
        # uncomment below line for mlflow tracking of local training
        ...
        with mlflow.start_run():
            ...
            mlflow.log_metrics(
                {
                    ...
                    "train_accuracy": np.mean(self.accuracy),
                    "val_accuracy": np.mean(self.val_accuracy),
                }
            )
            ...
            if tracking_url_type_store != "file":
                ...
                mlflow.pytorch.log_model(
                    self.model,
                    "model",
                    registered_model_name=self.config.params_model_name,
                )
            else:
...
class ModelTrainerPyTorch(object):
    ...
    def set_loaders(self):
        ...
        tokenizer_kwargs = dict(
            truncation=True, padding=True, max_length=30, add_special_tokens=True
        )
        ...
        train_tensor_dataset = self._tokenize_dataset(
            train_dataset_float,
            "Findings",
            "labels",
            auto_tokenizer,
        ...
        test_tensor_dataset = self._tokenize_dataset(
            test_dataset_float, "Findings", "labels", auto_tokenizer, **tokenizer_kwargs
        )
    ...
    def _tokenize_dataset(
        self, hf_dataset, sentence_field, label_field, tokenizer, **kwargs
    ):
    ...
    def predict(self, text):
        ...
        tokenize_text = self._tokenize_dataset(
            text, "sentence", "labels", auto_tokenizer, **tokenizer_kwargs
        )
    ...
    def log_into_mlflow(self):
        ...
        with mlflow.start_run():
            ...
            if tracking_url_type_store != "file":
                ...
                # https://mlflow.org/docs/latest/model-registry.html#api-workflow
                mlflow.pytorch.log_model(
                    self.model, "model", registered_model_name="nlp18Model"
                )
            else:
                mlflow.pytorch.log_model(self.model, "model")
